...
**Phải hiểu — riêng đề tài này:** Khác biệt functional correctness và physical closure, critical path/slack, utilization và congestion, PPA trade-off
...
**Mentor hỏi:** IP này đủ phức tạp để có research question nhưng vẫn đóng được không?
...
**Phải dựng — riêng đề tài này:** Chọn và viết/chuẩn hóa một IP số tham số hóa, testbench + regression
...
**Mentor hỏi:** Tại sao constraint này làm timing/area thay đổi?
...
**Phải dựng — riêng đề tài này:** automation scripts cho flow, script trích xuất PPA/timing
...
**Mentor hỏi:** Nếu layout pass DRC nhưng fail LVS nghĩa là gì?
...
**Thí nghiệm trọng tâm — riêng đề tài này:** Baseline RTL→GDSII, clock-period sweep, core-utilization sweep, architecture hoặc pipeline comparison, re-run để kiểm tra reproducibility
...
**Bằng chứng trọng tâm — riêng đề tài này:** waveform/regression pass, synthesis report, STA report, layout/GDS screenshot, DRC/LVS summary, CSV PPA sweep + scripts, Pareto/summary plots
...
**Mentor hỏi:** Kết quả nào là insight chứ không chỉ tool output?
...
**Ngưỡng riêng của đề tài — riêng đề tài này:** Có systematic design-space/PPA study; ít nhất 2 cấu hình/kiến trúc có so sánh công bằng; DRC/LVS sạch; có insight tái lập vượt quá demo flow.

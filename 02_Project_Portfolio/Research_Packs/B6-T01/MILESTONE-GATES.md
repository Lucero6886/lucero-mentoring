...
**Phải hiểu — riêng đề tài này:** why neural model is justified only after simple baseline, train/val/test split, data leakage, class imbalance
...
**Mentor hỏi:** NN học được gì mà feature threshold không làm được?
...
**Phải dựng — riêng đề tài này:** dataset pipeline, threshold baseline
...
**Mentor hỏi:** Có leakage giữa train/test theo frame/SNR không?
...
**Phải dựng — riêng đề tài này:** logistic/simple ML baseline, tiny MLP, quantized inference model, hardware-cost estimator
...
**Mentor hỏi:** AUC tăng có chuyển thành decoder gain không?
...
**Thí nghiệm trọng tâm — riêng đề tài này:** threshold vs logistic vs tiny MLP, feature ablation, model-size sweep, quantization sweep, ROC/PR, end-to-end activation/BLER/average-cost
...
**Bằng chứng trọng tâm — riêng đề tài này:** split manifest, learning curves, ROC/PR, ablation table, parameter/MAC/memory table, end-to-end adaptive-decoder benefit
...
**Mentor hỏi:** Quantization có làm mất lợi thế không?
...
**Ngưỡng riêng của đề tài — riêng đề tài này:** NN phải vượt simple baselines có ý nghĩa ở end-to-end decoder metric sau khi tính overhead; có ablation, quantization và complexity analysis.

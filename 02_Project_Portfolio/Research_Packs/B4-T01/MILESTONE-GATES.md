...
**Phải hiểu — riêng đề tài này:** why flipping may recover a frame, true first error vs propagated errors, retry mechanism, Tmax
...
**Mentor hỏi:** SCF sửa first error hay mọi error?
...
**Phải dựng — riêng đề tài này:** SCF software baseline, candidate selector
...
**Mentor hỏi:** Average complexity có ý nghĩa gì so với worst case?
...
**Phải dựng — riêng đề tài này:** retry controller model, optional RTL controller integration
...
**Mentor hỏi:** Tmax tăng đem lại diminishing returns khi nào?
...
**Thí nghiệm trọng tâm — riêng đề tài này:** BLER vs SNR, Tmax sweep, average attempts vs SNR, latency distribution, comparison SC/SCF/strong baseline where feasible
...
**Bằng chứng trọng tâm — riêng đề tài này:** BLER curves, attempt histogram, average/worst latency table, candidate-hit diagnostics, complexity accounting
...
**Mentor hỏi:** SCF có lợi phần cứng hơn SCL ở giả định nào?
...
**Ngưỡng riêng của đề tài — riêng đề tài này:** Có strong baseline, BLER–latency/complexity trade-off rõ, không chỉ reproduce algorithm; tốt hơn nếu có hardware-oriented modification/analysis.
